// /* eslint-disable no-unused-expressions */
// import { expect } from 'chai'
// import params from '../../test.config.js'
// import Pre from '../../../pre-post/pre.js'
// import Post from '../../../pre-post/post.js'
// import asyncPipe from '../../../../utils/async-pipe.js'

// describe('GET /api/v1/topics', () => {
//   let agent
//   const userData = { username: 'user test 1', password: 'AzEr12ù*' }
//   const universeData = { name: 'universe test 1', description: 'universeTest1\'s description', bIsPublic: false }
//   const topicData = { name: 'topic test 1' }

//   // eslint-disable-next-line no-undef
//   before(() => Pre.createBlankDatabaseWithEmptyTables(params))

//   beforeEach(async () => {
//     const ret = await asyncPipe(
//       Pre.createExpress
//     )()
//     agent = ret.agent
//   })
//   afterEach(() => Post.truncateTables())

//   it('should return the list of all the topics', async () => {
//     const { topic, universe } = await asyncPipe(
//       Pre.pipeCreateUserLogin,
//       Pre.createUniverse,
//       Pre.createTopic
//     )({ agent, topicData, universeData, userData })

//     return agent
//       .get('/api/v1/topics')
//       .expect(200)
//       .expect('Content-Type', /json/)
//       .then((response) => {
//         expect(response.body._links).to.have.all.keys(['self'])
//         expect(response.body.list).to.be.an('array').of.length(2)

//         expect(response.body.list[0]._links).to.have.all.keys(['self', 'universe', 'sub-topics'])
//         expect(response.body.list[0].name).to.equal('[OTTERWORLDS-TOPIC-SYSTEM]')
//         expect(response.body.list[0].id).to.be.an('number')
//         expect(response.body.list[0].order).to.equal(0)
//         expect(response.body.list[0].idUniverse).to.equal(universe.id)

//         expect(response.body.list[1]._links).to.have.all.keys(['self', 'universe', 'sub-topics'])
//         expect(response.body.list[1].id).to.equal(topic.id)
//         expect(response.body.list[1].name).to.equal(topicData.name)
//         expect(response.body.list[1].idUniverse).to.equal(universe.id)
//       })
//   })
// })
