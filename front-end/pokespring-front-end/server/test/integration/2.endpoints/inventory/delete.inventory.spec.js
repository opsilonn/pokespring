...
describe('DELETE /api/v1/inventories/:id', () => {
  ...
  it('should return the status 200 if we are authenticated as a user who can edit the universe of the selected inventory', async () => {
  ...
  it('should return the status 401 if we are authenticated as a user who can\'t edit the character the selected inventory', async () => {
    const { inventory } = await asyncPipe(
      Pre.pipeCreateUserUniverse,
      Pre.pipeCreateCharacterInventory
    )({ characterData, inventoryData, universeData, userData })
