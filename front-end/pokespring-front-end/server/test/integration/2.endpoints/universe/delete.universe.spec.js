...
describe('DELETE /api/v1/universes/:id', () => {
  let agent
  ...
  it('should return the status 200 if we are authenticated as the user owner of the selected universe', async () => {
  ...
  it('should return the status 401 if the selected universe doesn\'t exists', async () => {
    ...
    return agent
      .delete('/api/v1/universes/1')
      .expect(401)
  })

  it('should return the status 401 if we are authenticated but not as the user owner of the selected universe', async () => {
    const { universe } = await Pre.pipeCreateUserUniverse({ universeData, userData })
