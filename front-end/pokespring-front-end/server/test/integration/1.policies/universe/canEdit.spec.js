...
describe('Universe\'s policy canEdit', () => {
  ...
  it('should return true if we are authenticated as the owner of the universe which is public', async () => {
    ...
    expect(await UniversePolicy.canEdit(user.id, universe.id)).to.be.true
  })
  ...
  it('should return false if we are authenticated as a basic user invited in the universe which is public', async () => {
    const { universe } = await Pre.pipeCreateUserUniverse({ universeData, userData })
    ...
    expect(await UniversePolicy.canEdit(user.id, universe.id)).to.be.false
  })

  it('should return false if we are authenticated as a user not invited in the universe which is public', async () => {
  ...
  it('should return false if we are authenticated as a basic user invited in the universe which is private', async () => {
    const { universe } = await Pre.pipeCreateUserUniverse({ universeData: { ...universeData, bIsPublic: false }, userData })
  ...
  it('should return false if the universe doesn\'t exists', async () =>
    expect(await UniversePolicy.canEdit(0, 1)).to.be.false
  )
})
