...
export default async function postMapImage (req, res) {
  ...
  try {
    const pathURI = `universes/${req.query.universe}/articles/${req.params.id}`
    try {
    ...
    await sharp(image.path)
      ...
      .toFile(`./static-back/back/${pathURI}/article.jpg`)
    res.status(201).json(`/back/${pathURI}/article.jpg`)
  } catch (err) {
